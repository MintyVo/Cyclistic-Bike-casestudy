...
```{r }
bike_data1$weekday <- ordered(bike_data1$weekday, levels=c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"))
...
bike_data1 %>%    
  ...
  labs(x= 'Day of Week', y='Total Number of Rides', title='Rides per Day of Week', fill = 'Type of Membership') + scale_y_continuous(breaks = c(250000, 400000, 550000), labels = c("250K", "400K", "550K"))
```
...
bike_data1 %>%  
  ...
  labs (x ="Month", y = "Average Ride Length(min)", title = "Average Ride Length by Customer Type and Month", fill = "Type of Membership") + theme(axis.text.x = element_text(angle = 45))
```
...
bike_data1 %>%  
  ...
  labs(x = 'Bike Type', y = 'Number of Rentals', title ='Bike Type Breakdown', fill = 'Type of Membership') + scale_y_continuous(breaks = c(500000, 1000000, 1500000), labels = c("500K", "1Mil", "1.5Mil"))
...
1. Age and gender - This data can show the demographics of the existing customers which could be used to target others in similar groups

2. Marital status - This data can be used for promotional use to target entire families and not just individuals

3. Pricing of plans - This data can be used to optimize the cost benefit analysis for existing customers and potential new ones

4. Household data - This data could be used for targeting potential customers in certain income ranges and/or neighborhoods